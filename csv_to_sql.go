...
func main() {
	...
	CONEXAO = USUARIO + ":" + SENHA + "@tcp(" + IP + ":3306)/" + DATABASE
...
func parseArgsDoCMD() {
	...
	base := 		flag.String("base", DATABASE, "Nome da base MySQL.")
	tabela := 		flag.String("tabela", TABELA, "Nome da tabela MySQL.")
	delimitador := 	flag.String("d", string(DELIMITADOR), "Delimitador do csv.")
	max_conex := 	flag.Int("c", MAX_SQL_CONEX, "Número de conexões MySQL max: 150.")
	usuario := 		flag.String("u", USUARIO, "Usuário mysql.")
	senha := 		flag.String("s", SENHA, "Senha MySQL.")
	flag.Parse()
